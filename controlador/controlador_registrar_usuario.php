...
if (!empty($_POST["btnregistrar"])) {
    ...
    if (!empty($_POST["txtapellidopaterno"]) and !empty($_POST["txtapellidomaterno"]) and !empty($_POST["txtnombres"])  and !empty($_POST["txtDNI"]) and !empty($_POST["txtusuario"]) and !empty($_POST["txtpassword"]) ) {
         ...
         <script>
            $(function notificacion(){
                new PNotify({
                   title:"ERROR",
                   type:"error",
                   text:"El usuario <?=$usuario ?> ya se encuentra registrado en la base datos",
                   styling:"bootstrap3"
                })
            })
         </script>
        ...
        <?php } else {
            ...
            $registro=$conexion->query(" insert into usuario(apellido_paterno,apellido_materno,nombre,dni,usuario,password)values('$apellidopaterno','$apellidomaterno','$nombre','$dni','$usuario','$password') ");
            if ($registro==true) {?>
             <script>
                $(function notificacion(){
                    new PNotify({
